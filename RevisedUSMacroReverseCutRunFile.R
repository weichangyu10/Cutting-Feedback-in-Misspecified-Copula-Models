...
PAR.input <- c((TransformedPCorArray.input[,,1])[lower.tri(TransformedPCorArray.input[,,1])],c(TransformedPCorArray.input[,,2:5]), rep(log(0.3^2), 5),  rep(0, ncol(Y.input)*nrow(Y.input)) )
RankMAT.reverse.input <- cbind(rank(Y.input[,1], ties.method = "first"), rank(Y.input[,2], ties.method = "first"), rank(Y.input[,3], ties.method = "first"), rank(Y.input[,4], ties.method = "first"))

#NewLogJointPosteriorUSMacroReverseStage1cpp(RankMAT = RankMAT.reverse.input,PARsub = PAR.input, p = 4)
Optim.test <- optim(par = PAR.input, fn = NewLogJointPosteriorUSMacroReverseStage1cpp, method = "L-BFGS-B", hessian = TRUE, control = list(fnscale=-1),  RankMAT = RankMAT.reverse.input, p=4)
write.csv( x = data.frame( mu = Optim.test$par, C = sqrt(-1/diag(Optim.test$hessian))), file = "ReverseCutPosteriorStartingValues.csv")
ReverseCut.INIT = read.csv("ReverseCutPosteriorStartingValues.csv", header = TRUE)
VB.reverse.cut.Obj <- NewReverseCutPosteriorUSMacro.StageI(Y.DAT = Y.input, p = 4, mu.init = ReverseCut.INIT$mu, ReverseCut.INIT$C, maxITERS = 5000)
VB.reverse.cut.Stage2.Obj <- NewReverseCutPosteriorUSMacro.StageII(Y.DAT = Y.input, p = 4, mu.fix = colMeans(VB.reverse.cut.Obj$MuSTORE[4001:5000,1:75]),C.fix = colMeans(VB.reverse.cut.Obj$CSTORE[4001:5000,1:75]), mu.init = c(xi.input,log.omega.input, alpha.input,log.nu.input), C.init = rep(0.1,16), maxITERS = 10000)
...
for(t.train.end in 84:266){
  ...
  VB.reverse.cut.obj.temp <- NewReverseCutPosteriorUSMacro.StageI(Y.DAT = Y.input, p = 4, mu.init = VB.reverse.cut.Obj$MuSTORE[5000,], VB.reverse.cut.Obj$CSTORE[5000,], maxITERS = 5000)
  VB.reverse.cut.Stage2.Obj.temp <- NewReverseCutPosteriorUSMacro.StageII(Y.DAT = Y.input, p = 4, mu.fix = colMeans(VB.reverse.cut.obj.temp$MuSTORE[4001:5000,1:75]),C.fix = colMeans(VB.reverse.cut.obj.temp$CSTORE[4001:5000,1:75]), mu.init = c(xi.input,log.omega.input, alpha.input,log.nu.input), C.init = rep(0.1,16), maxITERS = 8000)
  ...
  MuFINAL.temp <- c(colMeans(VB.reverse.cut.Stage2.Obj.temp$MuSTORE[7501:8000,1:16]),colMeans(VB.reverse.cut.obj.temp$MuSTORE[4001:5000,1:70]))
  VarFINAL.temp <- c(colMeans(VB.reverse.cut.Stage2.Obj.temp$CSTORE[7501:8000,1:16]^2),colMeans(VB.reverse.cut.obj.temp$CSTORE[4001:5000,1:70]^2))
  ...
  for(l in 4991:5000){
    ...
    U3prime.temp <- pmin(pmax( rep(0.01,8), U3prime.temp ), rep(0.95,8))
    ...
    U4prime.temp <- pmin(pmax( rep(0.01,8), U4prime.temp ), rep(0.95,8))
  ...
  save.image("USMacroReverseCut.RData")
