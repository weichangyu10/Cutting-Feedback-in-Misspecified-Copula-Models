...
Optim.test1 <- optim(par = c(Marginal.MLE1[1], log(Marginal.MLE1[2]), Marginal.MLE1[3], log(Marginal.MLE1[4]) ), fn = NewLogJointPosteriorSkewTUSMacroCutStage1cpp, method = "L-BFGS-B", hessian = TRUE, control = list(fnscale=-1),  Yvec=Y.input[,1])
C.sub1 <- sqrt(diag((-solve(Optim.test1$hessian))))
mu.sub1 <- Optim.test1$par
VB.cut.Stage1.Obj1 <- NewCutIPosteriorSkewTMarginalUSMacro(vy = Y.input[,1], mu.init = mu.sub1, C.init = C.sub1, maxITERS = 5000)

Optim.test2 <- optim(par = c(Marginal.MLE2[1], log(Marginal.MLE2[2]), Marginal.MLE2[3], log(Marginal.MLE2[4]) ), fn = NewLogJointPosteriorSkewTUSMacroCutStage1cpp, method = "L-BFGS-B", hessian = TRUE, control = list(fnscale=-1),  Yvec=Y.input[,2])
C.sub2 <- sqrt(diag((-solve(Optim.test2$hessian))))
mu.sub2 <- Optim.test2$par
VB.cut.Stage1.Obj2 <- NewCutIPosteriorSkewTMarginalUSMacro(vy = Y.input[,2], mu.init = mu.sub2, C.init = C.sub2, maxITERS = 5000)

Optim.test3 <- optim(par = c(Marginal.MLE3[1], log(Marginal.MLE3[2]), Marginal.MLE3[3], log(Marginal.MLE3[4]) ), fn = NewLogJointPosteriorTruncatedSkewTUSMacroCutStage1cpp, method = "L-BFGS-B", hessian = TRUE, control = list(fnscale=-1),  Yvec=Y.input[,3])
C.sub3 <- sqrt(diag((-solve(Optim.test3$hessian))))
mu.sub3 <- Optim.test3$par
VB.cut.Stage1.Obj3 <- NewCutIPosteriorTruncatedSkewTMarginalUSMacro(vy = Y.input[,3], mu.init = mu.sub3, C.init = C.sub3, maxITERS = 40000)

Optim.test4 <- optim(par = c(Marginal.MLE4[1], log(Marginal.MLE4[2]), Marginal.MLE4[3], log(Marginal.MLE4[4]) ), fn = NewLogJointPosteriorTruncatedSkewTUSMacroCutStage1cpp, method = "L-BFGS-B", hessian = TRUE, control = list(fnscale=-1),  Yvec=Y.input[,4])
C.sub4 <- sqrt(diag((-solve(Optim.test4$hessian))))
mu.sub4 <- Optim.test4$par
VB.cut.Stage1.Obj4 <- NewCutIPosteriorTruncatedSkewTMarginalUSMacro(vy = Y.input[,4], mu.init = mu.sub4, C.init = C.sub4, maxITERS = 10000)
...
PAR.input <- c((TransformedPCorArray.input[,,1])[lower.tri(TransformedPCorArray.input[,,1])],c(TransformedPCorArray.input[,,2:5]), rep(log(0.3^2), 5) )

Mu.Fix.Sub <- c( colMeans(VB.cut.Stage1.Obj1$MuSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj2$MuSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj3$MuSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj4$MuSTORE[4001:5000,]) )
C.Fix.Sub <- c( colMeans(VB.cut.Stage1.Obj1$CSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj2$CSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj3$CSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj4$CSTORE[4001:5000,]) )

VB.cut.Stage2.Obj <- NewCutIPosteriorUSMacro.StageII(Y.DAT = Y.input, p = 4, mu.init = PAR.input, mu.fix = Mu.Fix.Sub, C.init = rep(0.1, length(PAR.input)), C.fix = C.Fix.Sub, maxITERS = 10000 )
PAR.input.temp <- colMeans(VB.cut.Stage2.Obj$MuSTORE[8001:10000,17:91])
...
for(t.train.end in 84:266){
  ...
  VB.cut.Stage1.Obj1.temp <- NewCutIPosteriorSkewTMarginalUSMacro(vy = Y.input[,1], mu.init = colMeans(VB.cut.Stage1.Obj1$MuSTORE[4001:5000,]), C.init = colMeans(VB.cut.Stage1.Obj1$CSTORE[4001:5000,]), maxITERS = 5000)
  VB.cut.Stage1.Obj2.temp <- NewCutIPosteriorSkewTMarginalUSMacro(vy = Y.input[,2], mu.init = colMeans(VB.cut.Stage1.Obj2$MuSTORE[4001:5000,]), C.init = colMeans(VB.cut.Stage1.Obj2$CSTORE[4001:5000,]), maxITERS = 5000)
  VB.cut.Stage1.Obj3.temp <- NewCutIPosteriorSkewTMarginalUSMacro(vy = Y.input[,3], mu.init = colMeans(VB.cut.Stage1.Obj3$MuSTORE[30001:40000,]), C.init = colMeans(VB.cut.Stage1.Obj3$CSTORE[30001:40000,]), maxITERS = 40000)
  VB.cut.Stage1.Obj4.temp <- NewCutIPosteriorSkewTMarginalUSMacro(vy = Y.input[,4], mu.init = colMeans(VB.cut.Stage1.Obj4$MuSTORE[8001:10000,]), C.init = colMeans(VB.cut.Stage1.Obj4$CSTORE[8001:10000,]), maxITERS = 10000)
  
  Mu.Fix.Sub.temp <- c( colMeans(VB.cut.Stage1.Obj1.temp$MuSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj2.temp$MuSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj3.temp$MuSTORE[30001:40000,]), colMeans(VB.cut.Stage1.Obj4.temp$MuSTORE[8001:10000,]) )
  C.Fix.Sub.temp <- c( colMeans(VB.cut.Stage1.Obj1.temp$CSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj2.temp$CSTORE[4001:5000,]), colMeans(VB.cut.Stage1.Obj3.temp$CSTORE[30001:40000,]), colMeans(VB.cut.Stage1.Obj4.temp$CSTORE[8001:10000,]) )
  
  VB.cut.Stage2.Obj.temp <- NewReverseCutIPosteriorUSMacro.StageII(Y.DAT = Y.input, p = 4, mu.init = PAR.input.temp, mu.fix = Mu.Fix.Sub.temp, C.init = rep(0.1, length(PAR.input.temp)), C.fix = C.Fix.Sub.temp, maxITERS = 10000 )
  ...
  MuFINAL.temp <- colMeans(VB.cut.Stage2.Obj.temp$MuSTORE[8001:10000,1:86])
  VarFINAL.temp <- colMeans(VB.cut.Stage2.Obj.temp$CSTORE[8001:10000,1:86]^2)
  ...
  for(l in 1:5000){
    ...
    U3prime.temp <- pmin(pmax( rep(0.01,8), U3prime.temp ), rep(0.90,8))
    ...
    U4prime.temp <- pmin(pmax( rep(0.01,8), U4prime.temp ), rep(0.90,8))
  ...
  save.image("USMacroForwardCut.RData")
